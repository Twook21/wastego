...
function Reports() {
  ...
  const reportsData = [
    { date: '10 Mei 2025', type: 'Plastik', weight: '2.5', collector: 'Eco Recycle', status: 'Selesai', statusColor: 'green' },
    { date: '8 Mei 2025', type: 'Kertas', weight: '1.8', collector: 'Green Partners', status: 'Selesai', statusColor: 'green' },
    { date: '5 Mei 2025', type: 'Elektronik', weight: '3.2', collector: 'E-Waste Solutions', status: 'Diproses', statusColor: 'yellow' }
  ]
  ...
  const ChartCard = ({ icon, title, children }) => (
    ...
    >
      <div className="flex items-center mb-3 sm:mb-4">
        ...
        <h3 className="text-lg sm:text-xl font-semibold text-teal-900 dark:text-white">{title}</h3>
      </div>
      {children}
    </motion.div>
  )
  ...
  const DataRow = ({ data, isLast }) => (
    <div className={`p-4 ${!isLast && 'border-b dark:border-gray-700'} hover:bg-gray-50 dark:hover:bg-gray-750`}>
      <div className="flex justify-between items-center mb-2">
        <span className="font-medium text-sm text-teal-900 dark:text-white">{data.type}</span>
        <span className={`px-2 py-1 bg-${data.statusColor}-100 text-${data.statusColor}-800 dark:bg-${data.statusColor}-800 dark:text-${data.statusColor}-100 text-xs rounded-full`}>
          {data.status}
        </span>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12 transition-colors duration-200">
      ...
      >
        ...
        <motion.p 
          ...
          variants={fadeIn}
        >
          Pantau dan analisis pengelolaan sampah untuk dampak positif lingkungan.
        </motion.p>
      </motion.div>
      ...
      <motion.div 
        className={`${activeTab === 'data' ? 'hidden md:grid' : 'grid'} grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8 mb-6 sm:mb-8`}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.3 }}
      ...
      >
        <ChartCard icon="pie-chart" title="Distribusi Jenis Sampah">
          <div className="h-48 sm:h-64 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">Grafik Distribusi Sampah</p>
          </div>
        </ChartCard>
        ...
        <ChartCard icon="trending-up" title="Trend Bulanan">
          <div className="h-48 sm:h-64 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center">
            {/* Placeholder for chart */}
            <p className="text-gray-500 dark:text-gray-400 text-sm">Grafik Trend Bulanan</p>
          </div>
        </ChartCard>
      </motion.div>
      ...
      <motion.div 
        className={`${activeTab === 'charts' ? 'hidden md:block' : 'block'} bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden`}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.3 }}
        variants={slideFromBottom}
      >
        <div className="flex items-center p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-teal-900 dark:text-white">Rekapitulasi Data</h3>
        </div>
              ...
              {reportsData.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 bg-${item.statusColor}-100 text-${item.statusColor}-800 dark:bg-${item.statusColor}-800 dark:text-${item.statusColor}-100 text-xs rounded-full`}>
                      {item.status}
                    </span>
                  </td>
                </tr>
      ...
      >
        ...
        <motion.p 
          ...
          variants={fadeIn}
        >
          Jadwalkan pengambilan sampah secara rutin untuk lingkungan yang lebih bersih.
        </motion.p>
        <motion.button 
